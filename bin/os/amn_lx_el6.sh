...
#                                                                      #
# This script purpose it to audit an Enterprise Linux 6 server.        #
#                                                                      #
# Version history:                                                     #
# +----------+------------+------+-----------------------------------+ #
...
FIL_INI_PKG="${DIR_TMP}/pkg_lx_el6.txt"
FIL_INI_CRON="${DIR_TMP}/crn_lx_el6.txt"
SCR_MK_INI="${DIR_TMP}/amn_mk_ini.sh"

# Default parameters
DEF_SYS_USR="root|bin|daemon|adm|lp|sync|shutdown|halt|mail|uucp|operator|games|gopher|ftp|nobody|vcsa|saslauth|postfix|sshd"
DEF_SYS_GRP="root|bin|daemon|sys|adm|tty|disk|lp|mem|kmem|wheel|mail|uucp|man|games|gopher|video|dip|ftp|lock|audio|nobody|users|utmp|utempter|floppy|vcsa|cdrom|tape|dialout|saslauth|postdrop|postfix|sshd"
DEF_IFS=${IFS}
DEF_CRON_DIR="0hourly|raid-check"
DEF_CRON_H="0anacron"
DEF_CRON_D="logrotate|makewhatis\.cron|rhsmd"
DEF_CRON_W=""
DEF_CRON_M=""
...
# Boot.
DEF_TARGET=$(cat /etc/inittab | grep "^id" | awk -F':' '{print $2}')
...
# Known host keys.
if [[ $(find / -type f -name "known_hosts" | wc -l) == 0 ]]; then
  KNOWN_HOSTS_FILES="none"
else
  for KNOWN_HOSTS in $(find / -type f -name "known_hosts"); do
...
# Authorized SSH keys.
if [[ $(find / -type f -name "authorized_keys" 2>/dev/null | wc -l) == 0 ]]; then
  AUTH_KEYS_FILES="none"
else
  for AUTH_KEYS in $(find / -type f -name "authorized_keys" 2>/dev/null); do
...
# File systems.
IFS=$'\n'
for FS in $(eval df -ThP ${EXC_FS_TYPE} | tail -n +2); do
...
IFS=$'\n'
for GROUP in $(yum grouplist | sed 's/^[[:alnum:]]/\n&/' | awk -v RS='' "/Installed Groups/" | tail -n +2 | sed 's/^[ \t]*//'); do
  PKG_GRPS+=( "${GROUP}" )
IFS=${DEF_IFS}
...
# Services.
for SERVICE in $(service --status-all 2>/dev/null | grep "is running" | awk '{print $1}'); do
  SVC_RUNNING+=( "${SERVICE}" )
done
...
if [[ $(ls /var/spool/cron/*) ]]; then
  for FILE in $(ls -1 /var/spool/cron/); do
    if [[ -s /var/spool/cron/${FILE} ]]; then
      CRON_USR+=( "${FILE}" )
      cp /var/spool/cron/${FILE} ${DIR_TMP_FIL}/cron_usr-${FILE}
    fi
  done
else
  CRON_USR=false
